...
exports.getAuditLogs = async (req, res) => {
  try {
    ...
    loans.forEach(l => {
      auditLogs.push({ id: `l-${l.id}`, action: `Created Loan (₹${l.principalAmount})`, performedBy: l.user?.name || 'Unknown', timestamp: l.createdAt });
    });
    payments.forEach(p => {
      auditLogs.push({ id: `p-${p.id}`, action: `Processed Payment (₹${p.totalCollected})`, performedBy: p.loan?.user?.name || 'System', timestamp: p.createdAt });
    });
